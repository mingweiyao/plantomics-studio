...
参数:
  ...
  summary_label: str       - 汇总文件标签(默认 "summary"),用于区分 raw/trimmed
                              输出 fastqc_<label>_summary.tsv

产出:
  <output_subdir>/<sample>/<sample>_fastqc.html / .zip   # 单个样本
  <output_subdir>/fastqc_<label>_summary.tsv             # 汇总(所有样本的关键指标)
"""
import io
import re
import zipfile
...
class FastqcRunner(BaseRunner):
    ...
    def run(self):
        ...
        # fastqc 每个并行任务是单线程进程,所以并行数 ≤ 全局配额即可
        ...
        # 按样本分组(每个 fastq 文件单独一个 fastqc 任务,但放到对应 sample 子目录)
        # 注意:这里不再 R1/R2 合并子目录,每个 fastq 一个独立报告
        # 因为汇总会按"file"作为粒度
        ...
        # 合并所有质控结果成一份 MultiQC HTML 报告(没装 multiqc 就软跳过)
    ...
    def _run_multiqc(self, out_dir: Path, label: str):
        """对 out_dir 跑 multiqc,把所有 fastqc 结果合并成一份 HTML。

        软失败:没装 multiqc 或跑挂了都只记一条日志,不影响整个任务成功
        (汇总 TSV 已经生成,基础信息不丢)。
        """
    ...
    def _extract_metrics(self, zip_path: Path) -> dict:
        """从 fastqc zip 解析 fastqc_data.txt + summary.txt 拿关键指标。"""
        result = {"filename": zip_path.stem.replace("_fastqc", "")}
        
        with zipfile.ZipFile(zip_path) as zf:
            # zip 内层目录 = <basename>_fastqc/
            # 找 fastqc_data.txt 和 summary.txt
            data_name = None
            summary_name = None
            ...
            if summary_name:
                with zf.open(summary_name) as f:
                    text = io.TextIOWrapper(f, encoding="utf-8").read()
                # 每行: "PASS\tPer base sequence quality\tfilename"
                for line in text.splitlines():
                    parts = line.split("\t")
                    if len(parts) >= 2:
