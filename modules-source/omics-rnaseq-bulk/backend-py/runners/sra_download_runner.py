...
每个 accession 独立,适合并行(下载是 IO bound,解压是 CPU bound)。
...
class SraDownloadRunner(BaseRunner):
    ...
    def run(self):
        ...
        # 把 并行数 × 单样本线程数 clamp 到全局 CPU 配额内
    ...
    def _extract_one(self, sra: Path, out_dir: Path, threads: int):
        ...
        产出:<out_dir>/<acc>/<acc>_1.fastq.gz / _2.fastq.gz / .fastq.gz
        
        注意:fasterq-dump 没有 --gzip 选项(老的 fastq-dump 才有,但单线程慢),
        所以走两步:fasterq-dump 出未压缩 fastq → pigz 并行压缩。
        中间产物未压缩,临时占盘多一倍,但 pigz 比 gzip 快 N 倍,整体反而快。
        """
        if not sra.exists():
        ...
        # tmp 走 Linux fs(NTFS 不支持 mmap/FIFO)
