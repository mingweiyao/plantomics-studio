# omics-ont-translatome

PlantOmics Studio 的 ONT 全长翻译组分析模块。

## 功能

基于 Oxford Nanopore 三代测序的全长翻译组分析流程:
...
- **基因组特征**: SSR 分析, 转录因子鉴定
- **⭐ Ref vs All 对比**: 双流程对比分析 (Venn/金字塔/富集对比)
...
```bash
sudo apt install ./plantomics-module-ont-translatome_X.X.X_amd64.deb
```

装到 `/opt/plantomics-studio/modules/omics-ont-translatome/`。
