"""gffcompare runner - novel transcript classification for DRS.

Compares query transcripts against a reference annotation to classify them
(known, novel isoform, intergenic, etc.).

Parameters:
  query_gtf: str        - Query transcript GTF
  reference_gtf: str    - Reference annotation GTF
  genome_fasta: str     - Optional genome FASTA for sequence comparison

Outputs:
  gffcmp.*                    - gffcompare output files
  novel_transcripts.tsv       - Novel transcript list
  gffcompare_summary.json     - Classification summary
"""
import json
...
class GffcompareRunner(BaseRunner):
    ...
    def run(self):
        ...
        prefix_path = out_dir / "gffcmp"
        cmd = ["gffcompare", "-r", reference_gtf, "-o", str(prefix_path)]
        ...
        tmap_files = list(out_dir.glob("gffcmp*.tmap"))
        stats_file = list(out_dir.glob("gffcmp*.stats"))
