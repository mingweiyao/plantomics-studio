...
def create_app() -> FastAPI:
    ...
    async def submit_basecall(req: SubmitJobBaseRequest):
        ...
        params: {pod5_dir, model?, kit?, threads?}"""
        if not req.params.get("pod5_dir"):
            raise HTTPException(400, "params.pod5_dir 必填")
        return _submit(JobKind.BASECALL.value, req)

    @app.post("/submit/nanofilt")
    async def submit_nanofilt(req: SubmitJobBaseRequest):
        """NanoFilt QC (q>=10).
        params: {fastq_files, min_qual=10, min_len=50}"""
        if not req.params.get("fastq_files"):
            raise HTTPException(400, "params.fastq_files 必填")
        return _submit(JobKind.NANOFILT.value, req)

    @app.post("/submit/polya-detect")
    async def submit_polya_detect(req: SubmitJobBaseRequest):
        """poly(A) proportion detection (Dorado --estimate-poly-a).
        params: {pod5_dir, model?, output_name?}"""
        if not req.params.get("pod5_dir"):
            raise HTTPException(400, "params.pod5_dir 必填(pod5 目录路径)")
        return _submit(JobKind.POLYA_DETECT.value, req)
    ...
    async def submit_minimap2_align(req: SubmitJobBaseRequest):
        """minimap2 alignment + read annotation.
    ...
    async def submit_polya_analysis(req: SubmitJobBaseRequest):
        ...
        params: {bam_files, gtf, polya_detect_dir?}"""
        if not req.params.get("bam_files"):
            raise HTTPException(400, "params.bam_files 必填")
        if not req.params.get("gtf"):
            raise HTTPException(400, "params.gtf 必填")
