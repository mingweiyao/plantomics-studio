"""omics-tail-iso-seq 模块 - Python 后端
...
def create_app() -> FastAPI:
    ...
    # ── Tail Iso-seq 具体 submit 端点 ──

    @app.post("/submit/nanofilt")
    async def submit_nanofilt(req: SubmitJobBaseRequest):
        """NanoFilt QC 过滤(q>=7)。
        params: {fastq_files: [str], min_qual: 7, min_len?, max_len?, threads?}"""
    ...
    async def submit_pychopper(req: SubmitJobBaseRequest):
        """Pychopper 全长转录本识别。
        params: {fastq_files: [str], sample_name: str, threads?}"""
        if not req.params.get("fastq_files"):
            raise HTTPException(400, "params.fastq_files 必填")
    ...
    async def submit_minimap2_align(req: SubmitJobBaseRequest):
        """minimap2 比对到参考基因组。
        params: {fastq_files: [str], genome_fasta: str, sample_names?: [str], threads?}"""
        if not req.params.get("fastq_files"):
            raise HTTPException(400, "params.fastq_files 必填")
    ...
    async def submit_pinfish(req: SubmitJobBaseRequest):
        """Pinfish 一致性转录本分析。
        params: {bam_file: str, genome_fasta: str, min_coverage?: float, threads?}"""
        if not req.params.get("bam_file"):
            raise HTTPException(400, "params.bam_file 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
    ...
    async def submit_stringtie(req: SubmitJobBaseRequest):
        """StringTie 冗余去除/组装。
        params: {bam_files: [str], gtf: str, sample_names?: [str], strand?, threads?}"""
        if not req.params.get("bam_files"):
            raise HTTPException(400, "params.bam_files 必填")
        if not req.params.get("gtf"):
            raise HTTPException(400, "params.gtf 必填(参考注释)")
    ...
    async def submit_transdecoder(req: SubmitJobBaseRequest):
        """TransDecoder CDS 预测。
        params: {candidate_gtf: str, genome_fasta: str, min_orf_aa?, single_best?}"""
    ...
    async def submit_salmon_quant(req: SubmitJobBaseRequest):
        """Salmon 转录本定量。
        params: {transcripts_fa: str, reads_files: [str], lib_type?: str, threads?}"""
    ...
    async def submit_suppa2(req: SubmitJobBaseRequest):
        """SUPPA2 可变剪接分析。
        params: {gtf: str, tpm_files: dict or [str], groups?, comparison?}"""
        if not req.params.get("gtf"):
    ...
    async def submit_fusion(req: SubmitJobBaseRequest):
        """融合转录本检测。
        params: {bam_files: [str], genome_fasta: str, annotation_gtf: str, threads?}"""
        if not req.params.get("bam_files"):
            raise HTTPException(400, "params.bam_files 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
    ...
    async def submit_lncrna_predict(req: SubmitJobBaseRequest):
        ...
        params: {candidate_gtf: str, genome_fasta: str, min_length?, threads?}"""
        if not req.params.get("candidate_gtf"):
            raise HTTPException(400, "params.candidate_gtf 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
    ...
    async def submit_polya(req: SubmitJobBaseRequest):
        """poly(A) 长度检测。
        params: {pod5_dir: str or fast5_dir: str, basecall_cfg?: str, threads?}"""
        if not req.params.get("pod5_dir") and not req.params.get("fast5_dir"):
    ...
    async def submit_polya_analysis(req: SubmitJobBaseRequest):
        """poly(A) 统计分析。
        params: {polya_tsv: str, groups?: dict, comparisons?: list}"""
