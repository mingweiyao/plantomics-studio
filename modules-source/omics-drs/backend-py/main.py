...
def create_app() -> FastAPI:
    ...
    async def submit_nanofilt(req: SubmitJobBaseRequest):
        """NanoFilt QC 过滤 + NanoStat/SeqKit 统计。
        params: {fastq_files: [str], min_qual: 10, min_len: 50, max_len?, threads?}"""
    ...
    async def submit_minimap2_align(req: SubmitJobBaseRequest):
        """minimap2 splice-aware 比对到参考基因组。
        params: {fastq_files: [str], sample_names?: [str], genome_fasta: str, threads?}
        -ax splice -uf -k14"""
        if not req.params.get("fastq_files"):
            raise HTTPException(400, "params.fastq_files 必填")
    ...
    @app.post("/submit/flair")
    async def submit_flair(req: SubmitJobBaseRequest):
        """Flair 全长转录本鉴定(align→correct→collapse)。
        params: {reads_fa: str, genome_fasta: str, annotation_gtf: str,
                 sample_name: str, threads?}"""
        if not req.params.get("reads_fa"):
            raise HTTPException(400, "params.reads_fa 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
        if not req.params.get("annotation_gtf"):
            raise HTTPException(400, "params.annotation_gtf 必填")
        return _submit(JobKind.FLAIR.value, req)

    @app.post("/submit/gffcompare")
    async def submit_gffcompare(req: SubmitJobBaseRequest):
        """gffcompare 新转录本分类。
        params: {query_gtf: str, reference_gtf: str, prefix?: str}"""
    ...
    async def submit_transdecoder(req: SubmitJobBaseRequest):
        """TransDecoder CDS 预测。
        params: {candidate_gtf: str, genome_fasta: str,
                 min_orf_aa?: int, single_best?: bool}"""
    ...
    async def submit_annot_7db(req: SubmitJobBaseRequest):
        """7 数据库功能注释(diamond blastp→Nr/Uniprot, hmmscan→Pfam, kofam_scan→KEGG)。
        params: {pep_fasta: str, threads?: int}"""
        if not req.params.get("pep_fasta"):
            raise HTTPException(400, "params.pep_fasta 必填(蛋白序列)")
    ...
    async def submit_salmon_quant(req: SubmitJobBaseRequest):
        """Salmon 转录本定量。
        params: {transcripts_fa: str, reads_files: [str],
                 lib_type?: str, threads?, sample_name?: str}"""
        if not req.params.get("transcripts_fa"):
            raise HTTPException(400, "params.transcripts_fa 必填")
        if not req.params.get("reads_files"):
            raise HTTPException(400, "params.reads_files 必填")
        return _submit(JobKind.SALMON_QUANT.value, req)

    @app.post("/submit/suppa2")
    async def submit_suppa2(req: SubmitJobBaseRequest):
        """SUPPA2 可变剪接分析。
        params: {gtf: str, tpm_files: dict or [str], groups?: dict,
                 comparison?: list}"""
        if not req.params.get("gtf"):
            raise HTTPException(400, "params.gtf 必填")
        if not req.params.get("tpm_files"):
            raise HTTPException(400, "params.tpm_files 必填")
        return _submit(JobKind.SUPPA2.value, req)

    @app.post("/submit/fusion")
    async def submit_fusion(req: SubmitJobBaseRequest):
        """融合转录本检测。
        params: {bam_files: [str], genome_fasta: str,
                 annotation_gtf: str, threads?}"""
    ...
    @app.post("/submit/lncrna-predict")
    async def submit_lncrna_predict(req: SubmitJobBaseRequest):
        """lncRNA 预测(CNCI+CPC2+PLEK 三工具, Venn 取交集)。
        params: {candidate_gtf: str, genome_fasta: str,
                 min_length?: int, threads?}"""
        if not req.params.get("candidate_gtf"):
            raise HTTPException(400, "params.candidate_gtf 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
        return _submit(JobKind.LNCRNA_PREDICT.value, req)

    @app.post("/submit/polya")
    async def submit_polya(req: SubmitJobBaseRequest):
        """poly(A) 长度检测(Dorado --estimate-poly-a)。
        params: {pod5_dir: str, basecall_cfg?: str, threads?}"""
        if not req.params.get("pod5_dir"):
            # Also accept fast5_dir
            if not req.params.get("fast5_dir"):
                raise HTTPException(400, "params.pod5_dir 或 fast5_dir 必填")
        return _submit(JobKind.POLYA.value, req)

    @app.post("/submit/polya-analysis")
    async def submit_polya_analysis(req: SubmitJobBaseRequest):
        """poly(A) 统计、表达相关、Mann-Whitney U 微分。
        params: {polya_tsv: str, groups?: dict, comparison?: list}"""
        if not req.params.get("polya_tsv"):
            raise HTTPException(400, "params.polya_tsv 必填")
        return _submit(JobKind.POLYA_ANALYSIS.value, req)

    @app.post("/submit/rna-mod")
    async def submit_rna_mod(req: SubmitJobBaseRequest):
        """RNA 修饰检测(m6A/m5C)。
        params: {bam_files: [str], genome_fasta: str,
                 mod_type?: str('m6A'|'m5C'|'both'), threads?}"""
        if not req.params.get("bam_files"):
            raise HTTPException(400, "params.bam_files 必填")
        if not req.params.get("genome_fasta"):
            raise HTTPException(400, "params.genome_fasta 必填")
        return _submit(JobKind.RNA_MOD.value, req)
