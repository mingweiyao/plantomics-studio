...
class JobKind(str, Enum):
    NANOFILT = "nanofilt"                    # NanoFilt QC (q>=10, len>=50) + ONT stats
    MINIMAP2_ALIGN = "minimap2_align"        # minimap2 splice-aware alignment
    FLAIR = "flair"                          # Flair consensus transcript (align→correct→collapse)
    GFFCOMPARE = "gffcompare"                # gffcompare novel transcript classification
    TRANSDECODER = "transdecoder"            # TransDecoder CDS prediction
    ANNOT_7DB = "annot_7db"                  # 7 database functional annotation
    SALMON_QUANT = "salmon_quant"            # Salmon transcript quantification
    SUPPA2 = "suppa2"                        # SUPPA2 alternative splicing analysis
    FUSION = "fusion"                        # Fusion transcript detection
    LNCRNA_PREDICT = "lncrna_predict"        # lncRNA prediction (CNCI+CPC2+PLEK Venn)
    POLYA = "polya"                          # poly(A) length detection (Dorado)
    POLYA_ANALYSIS = "polya_analysis"        # poly(A) stats/expression correlation/differential
    RNA_MOD = "rna_mod"                      # RNA modification detection (m6A/m5C)
