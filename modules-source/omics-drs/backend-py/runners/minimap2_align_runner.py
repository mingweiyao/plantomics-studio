...
splice-aware parameters (-ax splice -uf -k14) for direct RNA data.

Parameters:
  fastq_files: [str]   - Input FASTQ files (filtered or raw)
  sample_names: [str]  - Optional sample names
  genome_fasta: str    - Reference genome FASTA (index will be created if needed)
  out_bam: str         - Output BAM filename (default: <sample>.sorted.bam)
...
Outputs (per sample to output_dir/<sample>/):
  ...
  <sample>_align_stats.txt   - Alignment statistics from samtools flagstat
"""
...
class Minimap2AlignRunner(BaseRunner):
    ...
    def run(self):
        ...
        # minimap2 will auto-generate .mmi index if not present
        # but we can pre-index for better performance
        self.update(pct=2, stage="检查/准备 minimap2 索引")
        mmi_path = Path(genome_fasta + ".mmi")
        if not mmi_path.exists():
            self.log("minimap2 索引不存在,将自动创建")
        else:
            self.log(f"使用已有索引: {mmi_path}")

        # Build minimap2 base flags for DRS splice-aware alignment
        mm2_opts = ["-ax", "splice", "-uf", "-k14"]
        ...
        for i, (fq, name) in enumerate(zip(fastq_files, sample_names)):
            ...
            mm2_cmd = ["minimap2"] + mm2_opts + [genome_fasta, str(fq)]
            self.run_command(
                ["bash", "-c",
                 f"{' '.join(mm2_cmd)} > '{sam}' 2> >(grep -v '^[MTP]' >&2)"],
                indeterminate=True,
