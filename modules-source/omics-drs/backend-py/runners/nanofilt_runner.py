...
statistics with NanoStat and NanoStats/SeqKit.
...
Outputs (per sample to output_dir/<sample>/):
  ...
  <sample>_seqkit.txt      - SeqKit stats
...
class NanofiltRunner(BaseRunner):
    ...
    def run(self):
        ...
        for i, (fq, name) in enumerate(zip(fastq_files, sample_names)):
            ...
            # SeqKit stats
            self.update(pct=int(80 * i / n + 15),
                        stage=f"SeqKit 统计 ({i + 1}/{n})", detail=name)
            seqkit_out = sample_dir / f"{name}_seqkit.txt"
            self.run_command([
                "seqkit", "stats", str(filtered_fq),
                "--out-file", str(seqkit_out),
                "--threads", str(threads),
            ], indeterminate=True, heartbeat_stage=f"SeqKit {name}")
            ...
            sample_results.append({
                ...
                "seqkit_stats": str(seqkit_out) if seqkit_out.exists() else "",
            })
